...
double calculate_power_output(double Vw, double ww_pu) {
    ...
    double Lambda_i = 1.0 / (1.0 / (Lambda + 0.08 * beta) - 0.035 / (pow(beta, 3) + 1));
    
    double Cp = 0.5176 * (116.0 / Lambda_i - 0.4 * beta - 5) * exp(-21.0 / Lambda_i) + 0.0068 * Lambda;
    double Cp_pu = Cp / 0.48;
    
    double Pmw_pu = kp * Cp_pu * pow(Vw_pu, 3);
